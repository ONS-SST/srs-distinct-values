...
def comput_distinct_values(filepath: str, output_directory: str, max_rows_per_file: int) -> None:
	input_directory, file_name = os.path.split(filepath)
	file_name = file_name.rsplit('.', maxsplits=1)[0]
	
	output_directory = os.path.join(output_directory, file_name)
	
	data = dd.read_csv(filepath, dtype='object')
	
	os.mkdires(output_directory, exist_ok=True)
